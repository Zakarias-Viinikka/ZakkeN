...
internal object IntegrityCheckingUniffiLib {
    init {
        Native.register(IntegrityCheckingUniffiLib::class.java, findLibraryName(componentName = "my_yrs_lib"))
...
private fun uniffiCheckContractApiVersion(lib: IntegrityCheckingUniffiLib) {
    ...
    val scaffolding_contract_version = lib.ffi_my_yrs_lib_uniffi_contract_version()
...
public object FfiConverterTypeBossOfYrs: FfiConverter<BossOfYrs, Long> {
    ...
    override fun lift(value: Long): BossOfYrs {
        return BossOfYrs(UniffiWithHandle, value)
    }

    override fun read(buf: ByteBuffer): BossOfYrs {
        return lift(buf.getLong())
    }
...
public object FfiConverterTypeYrsError : FfiConverterRustBuffer<YrsException> {
    ...
    override fun write(value: YrsException, buf: ByteBuffer) {
        when(value) {
            ...
            is YrsException.Deadlock -> {
                buf.putInt(3)
                FfiConverterTypeDeadlockPrediction.write(value.`prediction`, buf)
                FfiConverterTypeErrorInfo.write(value.`info`, buf)
                Unit
            }
        }.let { /* this makes the `when` an expression, which ensures it is exhaustive */ }
