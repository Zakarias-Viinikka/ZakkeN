...
use crate::db_gui::components::confirm_modal::PendingConfirm;
                            ...
                            pending_confirm.set(Some(PendingConfirm {
                                ...
                                on_confirm: Callback::new(move |_| {
                                    ...
                                    spawn_local(async move {
                                        match local_sqlite_wrapper::delete_table(&table).await {
                                            Ok(msg) => {
                                                set_log_entries.update(|log| log.push(LogEntry {
                                                    tag: "delete_table".to_string(),
                                                    message: msg,
                                                }));
                                                match local_sqlite_wrapper::list_tables().await {
                                                    Ok(names) => set_table_names.set(names),
                                                    Err(e) => set_log_entries.update(|log| log.push(LogEntry {
