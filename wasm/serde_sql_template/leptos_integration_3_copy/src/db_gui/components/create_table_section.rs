...
impl ColumnRow {
    ...
    fn to_column_def(&self) -> CreateTableColumnDef {
...
pub fn CreateTableSection(set_table_names: WriteSignal<Vec<String>>) -> impl IntoView {
    ...
    let create_table = move |_| {
        ...
        let cols: Vec<CreateTableColumnDef> =
            columns.get().iter().map(ColumnRow::to_column_def).collect();

        spawn_local(async move {
            match local_sqlite_wrapper::create_table(&name, &cols).await {
                Ok(msg) => {
